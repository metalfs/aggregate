...
$(IMAGE_JSON):
	@echo '{ "target": "$(METAL_TARGET)", "stream_bytes": 64, "operators": { "aggregate": { "source": "./." } } }' > $@
...
SOLUTION_NAME ?= hls_operator_aggregate_sln
SOLUTION_DIR ?= hls_operator_aggregate_sln
...
srcs += hls_operator_aggregate.cpp
